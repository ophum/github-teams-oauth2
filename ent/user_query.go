...
func (uq *UserQuery) QueryCodes() *CodeQuery {
	query := (&CodeClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) loadCodes(ctx context.Context, query *CodeQuery, nodes []*User, init func(*User), assign func(*User, *Code)) error {
